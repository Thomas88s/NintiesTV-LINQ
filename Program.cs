...
namespace NinetiesTV
{
    class Program
    {
        ...
        **************************************************************************************************/
        ...
        {
            double minRating = shows.Min(s => s.ImdbRating);
            return shows.Where(s => s.ImdbRating > minRating).ToList();
        }
        ...
        {
            return shows.Where(s => s.Genres > 1).OrderByDescending(s => s.StartYear).Select(s => s.Name).ToList();
        }
